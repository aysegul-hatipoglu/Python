...
head = dataFrame1.head() #baştan 5 data
tail = dataFrame1.tail() #sondan 5 data
#içlerine sayı yazarsak o kadar değer gösterir.

# %%
# pandas basic method

print(dataFrame1.columns)#columnlarını verir.

print(dataFrame1.info())#dataframe hakkında bilgi verir.

print(dataFrame1.dtypes)

print(dataFrame1.describe())  # numeric feature = columns (age,maas): numeric alanları seçiyor.
...
print(dataFrame1.loc[:, "AGE"]) # AGE sütununa ait tüm satırları alırız

print(dataFrame1.loc[:3, "AGE"])# AGE sütununa ait 0dan 3e kadar satırları alırız
...
print(dataFrame1.iloc[:,2]) #name ile değilde indexi ile erişmemize izin veriyor.
...
dataFrame1.columns = [ each.lower() for each in dataFrame1.columns] #tüm columnları küçük harfe çevirir.

dataFrame1.columns = [each.split()[0]+"_"+each.split()[1] if(len(each.split())>1) else each for each in dataFrame1.columns] #boşluk içeren sütunlara _ ekler.

# %% drop and concatenating

dataFrame1.drop(["yeni_feature"],axis=1,inplace = True) # inplace = True olması değişikliği yap ve dataFrame1'e eşitle demektir.
